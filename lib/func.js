...
export default class {
    ...
    validate = (...opt) => {
        return new Promise(async (resolve) => {
            ...
            run = await corelight.getDefaultOptions(this, this.opt, this.default, overwrite, { dumpLevel, dumpFunc })
            if (run.error) {
            ...
            run = await corelight.isAvailableTypes(this, this.opt, this.types, fullMatch, { dumpLevel, dumpFunc })
            if (run.error) {
                resolve(this.err(run))
                return
            }
            
            run = await corelight.isAvailableValues(this, this.opt, this.values, { dumpLevel, dumpFunc })
            if (run.error) {
                resolve(this.err(run))
                return
            }
