...
export default class {
    constructor(...opt) {
        this._self = `func`
        this._initiator = opt[0]
        this._name = opt[1]
        this._argums = opt[2]
        this._dumpLevel = opt[3]
        this._dumpFunc = opt[4]
        this._default = {}
        this._types = {}
        this._values = {}
        this._opt = {}
    ...
    /**
     * @param {string} @argument - Default: 'Unknown'. For define function name.
     * @param {object_1} @argument - Default: {}. 'func' instance.
     * @param {object_2.dumpSplit {boolean}} @argument - Default: false. Define true, if need to split dump.
     * @param {object_2.dumpLevel {number}} @argument - Default: 0. For define dump level.
     * @param {object_2.dumpFunc  {function/null}} @argument - Default: null. For define dump function.
     * @param {any} @argument - Default: none. For define argums.
     * @param {array} @argument - Default: none. Can contain same options as above.
     * 
     * @returns {object} this
     */
    static init = (...opt) => {
            ...
            if (optValType === `Array`) {
                ...
                for (let optVal2 of optVal) {
                    ...
                    if (optVal2Type === `Object` && isSame(optVal2)) {
                        isClearArr = false
                        func = optVal2
                    } else if (optVal2Type === `Object` && isSettings(optVal2)) {
                        isClearArr = false
                    }
                }
                if (isClearArr && optVal.length) {
                    argums.push(optVal)
                } else {
                    for (let optVal2 of optVal) {
                        let optVal2Type = corelight.getType(optVal2)
        ...
        let self = new this(initiator, name, argums, dumpLevel, dumpFunc, dump)
        self.dump(`Started.`, 3)
        return self
    }
    ...
    /**
     * @param {undefined/boolean/number/string/null} @argument_1 - Default: ''. Data for dumping.
     * @param {number} @argument - Default: 3. For define dump level.
     * @param {function/null} @argument - Default: this.dumpFunc. For define dump function.
     * 
     * @returns {object} this
     */
    dump = (...opt) => {
        ...
        if (dumpLevel <= this.dumpLevel) {
            ...
            let dataType = corelight.getType(data)
            let typesToString = [ `Undefined`, `Boolean`, `Number`, `String`, `Null` ]
            if (!typesToString.includes(dataType)) {
                msg = `${this.initiator}->${this.name} (${type}): Function 'dump' error. Data type: '${dataType}' instead '${typesToString.join(`' ,'`)}'.`
            } else {
                msg = `${this.initiator}->${this.name} (${type}): ${data.toString()}`
            }
    ...
    /**
     * @param {any} @argument_1 - Default: null. Data for return.
     * @param {number} @argument - Default: 3. For define dump level.
     * @param {function/null} @argument - Default: this.dumpFunc. For define dump function.
     * 
     * @returns {object} this
    ...
    /**
     * @param {string} @argument_1 - Default: ''. For define error message.
     * @param {string} @argument_2 - Default: '0'. For define error code.
     * @param {number} @argument - Default: 3. For define dump level.
     * @param {function/null} @argument - Default: this.dumpFunc. For define dump function.
     * 
     * @returns {object} this
    ...
    /**
     * @param {string} @argument - Default: ''. For define option in this.opt from this.argums.
     * 
     * @returns {object} this
     */
    args = (...opt) => {
        if (!opt.length) {
            ...
            for (let argumValue of this.argums) {
                if (corelight.getType(argumValue) === `Object` && argumsLength === 1) {
            ...
            for (let optValue of opt) {
                if (corelight.getType(optValue) === `String`) {
                    if (this.argums.length) {
                        this.opt[optValue] = this._argums.shift()
                    } else {
    ...
    /**
     * @param {object_1.defaultMatch   {boolean}} @argument - Default: false.
     * @param {object_1.defaultPrimary {boolean}} @argument - Default: false.
     * @param {object_1.defaultPure    {boolean}} @argument - Default: false.
     * @param {object_1.typesMatch     {boolean}} @argument - Default: false.
     * @param {object_1.valuesMatch    {boolean}} @argument - Default: false.
     * @param {number} @argument - Default: this.dumpLevel. For define dump level.
     * @param {function/null} @argument - Default: this.dumpFunc. For define dump function.
     * 
